...
public class BranchFilterModule extends AbstractPipelineModuleImpl {
    ...
    protected void processMap(final URI map) {
        ...
        logger.debug("Remove job entries for filtered topics");
        removeFilteredJobEntries(doc.getDocumentElement());
...
//            }
    ...
    private void processAttributes(final Element elem, final Branch filter) {
        if (filter.resourcePrefix.isPresent() || filter.resourceSuffix.isPresent()) {
            ...
            if ((!href.isEmpty() || !copyTo.isEmpty()) && !scope.equals(ATTR_SCOPE_VALUE_EXTERNAL)) {
                ...
                dstBuilder.isSubtarget(true);                       // Flag for removeFilteredJobEntries
    ...
    private void removeFilteredJobEntries(final Element root) {
        // Remove job entries generated by splitBranches that aren't built by the filtered map.
        
        final Collection<FileInfo> jobFiles = job.getFileInfo();    // all files in job
        final Collection<Element> topicRefs = getTopicrefs(root);   // all topicrefs in map
        
        for (FileInfo jf: jobFiles) {
            String jfFormat = (jf.format == null) ? "" : jf.format;
            Boolean jfSubtarget = jf.isSubtarget || false;
            if (!jfFormat.equals("dita") || !jfSubtarget) {
                continue;                                           // only remove flagged dita targets
            }
            String jfURI = jf.uri.toString();
            Boolean doRemove = true;
            
            for (final Element e: topicRefs) {
                String topicHref = e.getAttribute(ATTRIBUTE_NAME_HREF);
                if (jfURI.equals(topicHref)) {
                    doRemove = false;
                    break;
                }
            }
            if (doRemove) {
                logger.info("Remove the following topic from further processing: " + jfURI);
                job.remove(jf);
            }
        }
    }
